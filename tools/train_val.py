...
from utils.eval_helper import dump, log_metrics, merge_together, performances, SaveAnomap
...
parser.add_argument("--config", default="experiments/RealIAD-full/config.yaml")
parser.add_argument("-e", "--evaluate", action="store_true")
parser.add_argument("--local_rank", default='0', help="local rank for dist")


def set_ckpt_local_cache(cache_dir: str):
    import torch

    if not osp.isabs(cache_dir):
        cache_dir = osp.abspath(cache_dir)
    torch.hub.set_dir(osp.join(cache_dir, 'torch', 'hub'))

    try:
        import huggingface_hub.constants
        huggingface_hub.constants.HUGGINGFACE_HUB_CACHE = osp.join(cache_dir, 'huggingface', 'hub')
    except ImportError:
        ...
...
def main():
    ...
    set_ckpt_local_cache(osp.dirname(osp.abspath(__file__)) + '/../pretrained/')
    ...
    if args.evaluate:
        validate(val_loader, model, collect=True)
        return
...
def anomap_to_score(preds: torch.Tensor) -> torch.Tensor:
    import torch.nn.functional as F
    avgpool_size = [16, 16]
    preds = F.avg_pool2d(preds, avgpool_size, stride=1)
    return preds.reshape(preds.shape[0], -1).max(dim=1).values


def validate(val_loader, model, collect=False):
    if not collect and config.evaluator.get('no_metrics', False):
        return {config.evaluator.key_metric: 0.}
    ...
    collectors = {cat: SaveAnomap(osp.join(config.save_path, f'{cat}.pkl'),
                                  config.dataset.input_size)
                  for cat in {sample['clsname'] for sample in val_loader.dataset.metas}
                  } if collect else dict()

    with torch.no_grad():
        for i, input in enumerate(val_loader):
            # forward
            outputs = model(input)
            if not config.evaluator.get('no_metrics', False):
                dump(config.evaluator.eval_dir, outputs)
            for cat, collector in collectors.items():
                sel = [idx for idx, clsname in enumerate(outputs['clsname'])
                       if clsname == cat]
                if len(sel) == 0:
                    continue
                sel_t = torch.tensor(sel, dtype=torch.int64, device=outputs['pred'].device)
                anomaly_maps = torch.index_select(outputs['pred'], dim=0, index=sel_t)
                scores = anomap_to_score(anomaly_maps)
                collector.record_batch(
                    anomaly_maps, scores,
                    [outputs['filename'][idx] for idx in sel],
                    [outputs['maskname'][idx] for idx in sel],
                )
    ...
    if rank == 0 and config.evaluator.get('no_metrics', False):
        logger.info(" * Loss {:.5f}\ttotal_num={}".format(final_loss, total_num.item()))
        ret_metrics.update({config.evaluator.key_metric: 0.})
    ...
    print(f'rank {rank} before barrior')
    dist.barrier()
    print(f'rank {rank} after barrior')
    model.train()
    return ret_metrics
